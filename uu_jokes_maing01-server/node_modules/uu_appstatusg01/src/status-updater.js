...
class StatusUpdater {
  constructor() {
    ...
    // 9. System sets nodeName from environment (nodeName format example:   "nodeName": "uuappg01_async-uu_script_engineg02_async-1617198494548.1.nkvnwdhr0k5w5pvcw5x2vsfpj"),
    this.nodeName =
      process.env.UU_CLOUD_NODE_NAME ||
      `uuappg01-${APPINFO.uuSubApp.replace("-server", "")}-${new Date().getTime()}.1.${new Date().getTime()}`;

    // 10. System sets subAppName from environment (subAppName format example: "uuScriptEngineg02")
    ...
    // 14. System sets runtime stack code
    this.runtimeStackCode = process.env.UU_CLOUD_RUNTIME_STACK_CODE || "";

    // 15. Systems sets node start time
    this.uuCloudNodeStartTime = NODE_START_TIME;

    // 16. Systems sets progressDataHistoryCount
    ...
    // 17. Systems sets uuCloudUniverse, uuCloudAppResourcePool
    ...
    // 18. Systems sets uuCloudRegion, uuCloudResourcePool, uuCloudResourceGroupCode
    ...
    // 19. Systems sets schema version
  ...
  async _checkExistingNode(session, nodeNumberInNodeSet, nodeSetUniqueId) {
    // 6.B.2.A.3. node number 1 in each nodeSet checks if exist progressBus and old progresses
    let progressBus;
    if (nodeNumberInNodeSet === "1") {
      // 6.B.2.A.3.A.1. System tries to get progressBus with all progresses
      try {
        // 6.B.2.A.3.A.1.A.
        ...
        progressBus = await AppClient.get(this.progressUri, { code: this.progressBusCode }, { session });
      } catch (e) {
        // 6.B.2.A.3.A.1.B.
      ...
      // 6.B.2.A.3.A.1.A.3. System checks for progresses and removes old ones if necessary
      if (progressBus && progressBus.progressList.length) {
        // 6.B.2.A.3.A.1.A.3.A.

        // 6.B.2.A.3.A.1.A.3.A.1. For each progress
        for (let progress of progressBus.progressList) {
          // 6.B.2.A.3.A.1.A.3.A.1.1. System parses node ids from progress.code
          let nodeIdsFromProgress = progress.code.split("_");
          let _nodeSetUniqueId = progress.code.split("_")[0];

          // 6.B.2.A.3.A.1.A.3.A.1.2. System checks if app is deployed after undeploy or redeployed with different version
          if (
            (nodeSetUniqueId !== _nodeSetUniqueId && nodeIdsFromProgress.length < 4 && this.spp === SPP_DEFAULT) ||
            (nodeSetUniqueId !== _nodeSetUniqueId && this.spp === nodeIdsFromProgress[3]) ||
            (progress.data && progress.data.uuSubAppVersion !== APPINFO.uuSubAppVersion &&
              nodeIdsFromProgress.length < 4 &&
              this.spp === SPP_DEFAULT) ||
            (progress.data && progress.data.uuSubAppVersion !== APPINFO.uuSubAppVersion && this.spp === nodeIdsFromProgress[3])
          ) {
            //6.B.2.A.3.A.1.A.3.A.1.2.A.

            LOGGER.debug(`Deleting old progress representing node of previous deployment(code ${progress.code}).`);

            // 6.B.2.A.3.A.1.A.3.A.1.2.A.1. System prepares progressEndDtoIn for ending progress.
            let progressEndDtoIn = {
              code: progress.code,
              lockSecret: LOCKSECRET,
            };

            await this._endProgress(progressEndDtoIn, session);
            await this._deleteProgress(progress, session);
          }
        }
      }
    }
