...
class Jwt {
  ...
  async createToken(claims, privateKey) {
    ...
    if (SubtleCrypto && !this._disableWebcryptoApi) {
      ...
      signature = await SubtleCrypto.sign(DEFAULT_KEY_PARAMS, cryptoKey, Buffer.from(signedData));
    } else {
  ...
  async _verifyToken(headers, claims, tokenParts, verifyOpts = {}) {
    ...
    if (SubtleCrypto && !this._disableWebcryptoApi) {
      try {
        ...
        isValid = await SubtleCrypto.verify(DEFAULT_KEY_PARAMS, cryptoKey, signature, Buffer.from(signedData));
      } catch (e) {
