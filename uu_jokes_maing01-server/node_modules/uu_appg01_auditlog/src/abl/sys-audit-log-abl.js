...
class SysAuditLogAbl {
  ...
  async createAuditLogRecord(dtoIn) {
    ...
    if (this.schemaVersion === SCHEMA_0_9_0) {
      auditLogRecord.eventTypeCode = dtoIn.logTypeCode || dtoIn.eventTypeCode;
      auditLogRecord.parameters = dtoIn.parameters;
      if (identity) {
      ...
      auditLogRecord.logTypeCode = dtoIn.logTypeCode || dtoIn.eventTypeCode;
      auditLogRecord.logData = dtoIn.parameters;
    ...
    if (this.appLogStoreUri) {
      ...
      if (this.appLogStoreNotTransferedCount >= this.appLogStoreTransferAfterCount) {
        ...
        this.mutex.runOneAtTime(this.transfer.bind(this), uuAppErrorMap).catch(e => this.logger.error(`Failed to call sys/logRecord/transfer command.`, e));
      } else if (this._checkTimeTransferCondition(this.appLogStoreLastTransferDate)) {
        // check time condition
        this.mutex.runOneAtTime(this.transfer.bind(this), uuAppErrorMap, true).catch(e => this.logger.error(`Failed to call sys/logRecord/transfer command.`, e));
      }
    }
  ...
  _parseUseCaseContextAttributes() {
    ...
    let ctxAuditMap = UseCaseContext.getUseCaseProperties().getAttribute("useCaseContextAttributesAuditMap");
